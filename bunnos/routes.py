...
"""
from bunnos import app
from flask import render_template, url_for, redirect, request, flash,jsonify
from bunnos.models import User, Meat_Rabbit, Breeding_Rabbit, Escrow_buyer, Escrow_seller
from bunnos.forms import RegisterForm, PostAdForm, SignInForm, PostAdForm2, SearchForm, BuyerEscrowForm, SellerEscrowForm
...
def home_page():
    ...
    breed_posts = Breeding_Rabbit.query.order_by(Breeding_Rabbit.date_created.desc()).limit(10)
    meat_posts = Meat_Rabbit.query.order_by(Meat_Rabbit.date_created.desc()).limit(10)
    return render_template('home.html', breed_posts=breed_posts,
                               meat_posts=meat_posts)
...
def profile_page():
    ...
    return render_template('profile.html',parent_list=parent_list, parent_meat=parent_meat)
...
def search():
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            ad_breed = ad_breed.filter(Breeding_Rabbit.description.like('%' + ad_search + '%'))
            ad_meat = ad_meat.filter(Meat_Rabbit.description.like('%' + ad_search + '%'))
            ad_breed = ad_breed.order_by(Breeding_Rabbit.title).all()
            ad_meat = ad_meat.order_by(Meat_Rabbit.title).all()
            return render_template("search.html",
                                   form=form, searched=ad_search,
                                   ad_breed=ad_breed, ad_meat=ad_meat)


@app.route('/ad_posts_breed', methods=['POST', 'GET'])
...
@app.route('/ad_posts_meat', methods=['POST', 'GET'])
def meat_posts_page():
    """ """
...
@app.route('/ad_posts/breed/<int:id>')
def breed_post(id):
    """ """
...
@app.route('/ad_posts/meat/<int:id>')
def meat_post(id):
    """ """
...
@app.route('/escrow/buyer', methods=['POST', 'GET'])
@login_required
def escrow_buyer():
    """ """
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            trade_to_create = Escrow_buyer(advert_id=form.advert_id.data,
            ...
            return redirect(url_for('escrow_page'))
    return render_template('escrow-buyer.html', form=form,  buyer_info=buyer_info)


@app.route('/escrow/seller', methods=['POST', 'GET'])
@login_required
def escrow_seller():
    """ """
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            blob_file = file.read()
            file_string = base64.b64encode(blob_file).decode('utf-8')
            trade_to_create = Escrow_seller(advert_id=form.advert_id.data,
                                            price_agreed=form.price_agreed.data,
                                            filename=file.filename,
                                            data=file_string,
...
@app.route('/escrow/delivery<int:id>', methods=['GET', 'DELETE'])
@login_required
def delivery(id):
    """ """
